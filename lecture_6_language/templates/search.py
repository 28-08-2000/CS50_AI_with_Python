...
def load(data, directory):
    ...
    for filename in os.listdir(directory): 
        with open(os.path.join(directory, filename), encoding="utf-8") as f:  # use  encoding="utf-8"
...
def match_template(template, corpus):
    ...
    regex = f"{prefix}((?:(?!{prefix}).){{0,40}}?){middle}(.{{0,40}}?){suffix}"   # BUG :: add f before first quotation mark
    results = re.findall(regex, corpus)
    if template["order"]:
        return results
    else:
